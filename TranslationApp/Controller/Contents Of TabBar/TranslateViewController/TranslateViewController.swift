...
class TranslateViewController: UIViewController, UITextViewDelegate {
    ...
    @IBOutlet weak var translateTextView1: UITextView!
    @IBOutlet weak var translateTextView2: UITextView!
    ...
    override func viewDidLoad() {
        ...
        setButton1(button: [selectFolderButton], pointSize: 18, weight: .medium, scale: .medium, systemName: ["square.and.arrow.down"], borderWidth: 1, borderColor: UIColor.systemBlue.cgColor, cornerRadius: 10)
        ...
        setButton1(button: buttonArr, pointSize: 20, weight: .regular, scale: .small, systemName: systemNameArr , borderWidth: nil, borderColor: nil, cornerRadius: nil)
        
        
        setButton2(button: [changeLanguageButton, saveButton, translateButton], borderColor: UIColor.systemBlue.cgColor, borderWidth: 1, cornerRadius: 10)
        ...
        setTranslateTextViewAndView()
        translateTextView1.delegate = self
        ...
        if let folderNameString = self.folderNameString{
            ...
            saveButton.setTitle("\(folderNameString)　へ保存する", for: .normal)
        }
    ...
    func setTranslateTextViewAndView(){
        ...
        translateTextView1.layer.borderColor = color
        translateTextView1.layer.borderWidth = 2
        translateTextView1.clipsToBounds = true
        
        translateTextView2.layer.borderWidth = 2
        translateTextView2.layer.borderColor = color
        translateTextView2.clipsToBounds = true
        
        view1.layer.borderWidth = 2
        view1.layer.borderColor = color
        
        view2.layer.borderWidth = 2
        view2.layer.borderColor = color
    }
    ...
    func setDoneTooBar(){
        ...
        let someArr = [self.translateTextView1, self.translateTextView2]
    ...
    @objc func doneButtonTaped(sender: UIButton){
        translateTextView1.endEditing(true)
        translateTextView2.endEditing(true)
    }
...
//        navigationBarのタイトルを設定
        ...
        if let folderNameString = self.folderNameString {
            ...
            if FolderNameArr.contains(folderNameString){
                saveButton.setTitle("保存先▷\(folderNameString)", for: .normal)
                saveButton.isHidden = false
                saveButton.isEnabled = true
            } else {
                saveButton.isEnabled = false
    ...
    @IBAction func translateButton(_ sender: Any) {
            if self.translateTextView1.text == "" {
                ...
                translateTextView1.layer.borderColor = UIColor.red.cgColor
    ...
    func textViewDidChange(_ textView: UITextView) {
        ...
        if textView == translateTextView1 {
            if translateTextView1.text != "" {
                self.label1.text = ""
            } else {
        ...
        func translateEnglish(){
            ...
            let parameters: [String: String] = [
                "text": self.translateTextView1.text,
                "auth_key": authKey,
                "source_lang" : "EN",
                "target_lang" : "JA"
            ]
        ...
        @IBAction func SaveButton(_ sender: Any) {
            if self.translateTextView2.text != "" && self.translateTextView2.text != "" {
                if let folderNameString = self.folderNameString {
                    ...
                    let translationArr = realm.objects(Translation.self)
...
//                    HistoryViewContoller(翻訳履歴画面）用にHistoryモデルクラスへ保存
                    ...
                    if self.translateTextView1.layer.borderWidth == 2 {
                        ...
                        translateTextView1.layer.borderWidth = 2
                        translateTextView1.layer.borderColor = UIColor.systemGray4.cgColor
                    }
                    
                    if self.translateTextView2.layer.borderWidth == 2 {
                        
                        translateTextView2.layer.borderWidth = 2
                        translateTextView2.layer.borderColor = UIColor.systemGray4.cgColor
                    }
                }
    ...
    @IBAction func selectFolderButton(_ sender: Any) {
        let selectFolderForTranslateViewContoller = self.storyboard?.instantiateViewController(withIdentifier: "FolderList") as! SelectFolderForTranslateViewContoller
    ...
    @IBAction func copyButton1(_ sender: Any) {
        copyTextView(textView: self.translateTextView1, button: copyButton1)
    }
    
    
    @IBAction func copyButton2(_ sender: Any) {
        copyTextView(textView: self.translateTextView2, button: copyButton2)
    }
...
//　音声再生
    @IBAction func volumeButton1(_ sender: Any) {
        speak(textView: self.translateTextView1)
        self.numberForVolumeButton2 = 0
    }
    
//    音声再生
    @IBAction func volumeButton2(_ sender: Any) {
        speak(textView: self.translateTextView2)
