//
//  ThirdPhraseWordViewController.swift
//  TranslationApp
//
...
// isChecked = 2 represents image icon whose system name is 'star.leadinghalf.filled'
class ThirdPhraseWordViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    private func appendValuesToArrays() {
        ...
        self.phraseWordArr = try! Realm().objects(PhraseWord.self).sorted(byKeyPath: "date", ascending: true).filter("isChecked == 2")
    ...
    @objc func tappedCheckMarkButton(_ sender: UIButton) {
        let alert = UIAlertController(title: "登録", message: "'星0'へ登録しますか？", preferredStyle: .alert)
