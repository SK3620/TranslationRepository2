...
class PronunciationViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    private func listenerAndGetDocuments() {
        ...
        if let user = Auth.auth().currentUser {
            let postsRef = Firestore.firestore().collection(FireBaseRelatedPath.PostPath).order(by: "postedDate", descending: true).whereField("topic", arrayContains: "発音")
...
extension PronunciationViewController: MFMailComposeViewControllerDelegate {
