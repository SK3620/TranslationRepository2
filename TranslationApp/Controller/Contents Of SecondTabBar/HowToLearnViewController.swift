//
//  HowToLearnViewController.swift
//  TranslationApp
//
...
class HowToLearnViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    private func listenerAndGetDocuments() {
        ...
        if let user = Auth.auth().currentUser {
            let postsRef = Firestore.firestore().collection(FireBaseRelatedPath.PostPath).order(by: "postedDate", descending: true).whereField("topic", arrayContains: "学習法")
...
extension HowToLearnViewController: MFMailComposeViewControllerDelegate {
